...
def hit_or_stand(deck, hand):
    ...
    while True:
        answer = input("Do you to to Hit or Stand? (H/S) ")
        if answer[0].lower() == 'h':
            hit(deck, hand)
...
def show_all(player, dealer):
    ...
    print(f"Dealer Hand = {dealer.value}")
    print("\nPlayer's Hand:", *player.cards, sep='\n')
    ...
    while True:
        ...
        if new_game[0].lower() == 'y':
            if player_chips.total == 0:
                buy = input("You out of chips! would you like to buy more chips? Enter 'y' or 'n' ")
                if buy[0].lower() == 'y':
                    chips_num = int(input("How much? "))
                    player_chips = Chips(chips_num)
                    playing = True
                    continue
                else:
